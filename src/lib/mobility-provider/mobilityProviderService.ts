...
const analyticsObserver = AnalyticsSystem.getInstance();
const mapObserver = new MapDisplay();

function createObservedResource(vehicleId: string, initialState: ResourceState) {
  const resource = new BookableResource(vehicleId, initialState);
  resource.attach(analyticsObserver);
  resource.attach(mapObserver);
  return resource;
}
...
class MobilityProviderService {
  ...
  async addVehicle(input: CreateVehicleInput) {
    const resourceState = createObservedResource("draft-resource", "Available");
  ...
  async updateVehicle(vehicleId: string, input: UpdateVehicleInput) {
    ...
    if (input.state !== undefined) {
      ...
      const bookableResource = createObservedResource(vehicleId, existing.state);
  ...
  async removeVehicle(vehicleId: string) {
    ...
    const bookableResource = createObservedResource(vehicleId, vehicle.state);
